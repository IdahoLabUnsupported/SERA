...
c
c modified Aug 10,1992 by F.J.Wheeler to also output equal area
c cosine cuts for Monte Carlo source
c only checked for nang = 131

      IMPLICIT REAL*8 (a-h,o-z)
      PARAMETER(nang=131,nngp=47,ntot=67,jup=17)
...
c jup:  17 for nang=131, 6 for nang=48

      dimension phin(nang),phig(nang),phit(nang)
     *,phi(nang),aphi(100,nang)
...
c write xgraph plot file for angular fit 
      ...
      IMPLICIT REAL*8 (a-h,o-z)
      CHARACTER pltfile*12,add*201
      DIMENSION xcut(ncut),cur(npolar),sumw(npolar)
      DATA add/'00100200300400500600700800901001101201301401501601701801
     *902002102202302402502602702802903003103203303403503603703803904004
     *104204304404504604704804905905105205305405505605705805906006106206
     *3064065066067'/
      ...
      WRITE(11,100) igp
  100 FORMAT('@    title  "Angular fit for group ',I4,'"',/
     *,'@    title font 4',/
     *,'@    title size 1.500000',/
     *,'@    title color 1',/
     *,'@    title linewidth 2',/
     *,'@    xaxis  label "(cosine of polar angle)"',/
     *,'@    yaxis  label "angular current intensity (dort)"',/
     *,'@    yaxis  ticklabel char size 0.75',/
     *,'@TYPE  xy')
