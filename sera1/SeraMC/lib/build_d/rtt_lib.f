c***********************************************************************
     ...
     * ,rec4_max,gamline_max
       ...
       PARAMETER (num_iso_max=50,ngrp_max=100,adum_max=11000,
     * nrr_max=500,irec3_max=3000,rec4_max=1500)    
...
c READ ISOTOPE DATA
  ...
  150 READ(15,end=190) ibcd,nidar,ltot,iwa,iwf,iws,jframe,ltype,iwr
...
c reading kermas directly
      ...
      DO 300 ig=1,num_fast_gps
        siga = 0.0
        IF(iwa .NE. 0) siga = adum(ig)
        j2 = ig
        IF(iwf .NE. 0) j2 = j2 +num_fast_gps
        sigs = 0.0
...
c***********************************************************************
      ...
      INTEGER*4 adum_max,rec4_max
       PARAMETER (ngrp_max=100,adum_max=11000,nrr_max=500
...
c*      write(6,200) jscat,nbound,head
  ...
  202 FORMAT(//,'**ERROR--could not fine thermal isotope',I6
