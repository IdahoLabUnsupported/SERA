...
void generic_resize_image(unsigned char *input, unsigned char *output, unsigned int inwidth, unsigned int inheight, unsigned int outwidth, unsigned int outheight, int pres_aspect_rat) {

    static unsigned int xcoorout, ycoorout, yindex;
    static float xcoorin, xstart, ycoorin;
    static unsigned char *data, *output_ptr, *input_ptr;
    static float rat_x, rat_y, ratio;
    static float inw_over_outw, inh_over_outh;
    static unsigned int * xarr, *xptr;
    int freeme = 0;
	...
	if (ratio<0.98) {
		...
		/*		debug("Converting to preserve aspect ratio.\n");
	...
	/* else if input==output, we don't have to do anything */
        DEBUG_TRACE_OUT printf("Leaving generic_resize_image\n");
	return;
    }
    ...
    /* No need to precompute y's as we'll only use them once -- but
     * it would be a good idea to precompute x's
     */
    xarr = (unsigned int *) MT_malloc(outwidth*sizeof(unsigned int));
    xcoorin = 0.5*inw_over_outw;
    for (xcoorout=0; xcoorout<outwidth; xcoorout++) {
      xarr[xcoorout]=(unsigned int)xcoorin;
      xcoorin+=inw_over_outw;
    }

    /* Find corresponding array indices of x and y in the input
     * images.  The addition of 0.5 is to move to the 'center'
     * of the pixel.  We now just increment these as we loop.
     */
    ycoorin = 0.5*inh_over_outh;
    output_ptr = output;
    for (ycoorout=0; ycoorout<outheight; ycoorout++) {
      yindex = ((int)ycoorin)*inwidth;
      xptr = xarr;
      input_ptr = data+yindex;
      for (xcoorout=0; xcoorout<outwidth; xcoorout++) {
	/* To set each pixel in the output, find the closest pixel
	 * in the input
	 */

	*(output_ptr++)=*(input_ptr+*(xptr++));
      }
      ycoorin+=inh_over_outh;
    }
    if (freeme) /* in this case, made a copy of input -- must free */
      MT_free((void*)data);
    MT_free((void*)xarr);
