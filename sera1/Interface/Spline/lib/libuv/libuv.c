/* This version of libuv.c frozen for Spline directory on 4-29-98
 */
...
FILE * read_uvh_minimal_fptr(geom_info_t *, char *, int);
int process_region_property(int, geom_info_t *, char *, char *);
int UV_readln(FILE *, char *, int);
int UV_readln_skip_comments_and_trim(FILE *, char *, int);
void UV_break_into_lower_key_and_value(char *, char *, char **, char **);
int UV_read_next_key_and_value(FILE *, char *, int, char **, char **);
int UV_read_string_value_for_key(FILE *, char *, char *, int);
int UV_trim_string(char *);
int read_uvval_for_body(FILE *, char *);
void read_body_information(FILE *, geom_info_t *, char *);
void read_constraint_information(FILE *, geom_info_t *, char *);
void read_fiducial_information(FILE *, geom_info_t *, char *);
void UV_make_lower_string(char *); 
int int_rnd(float);
int int_rnd_down(float);
/*float subtract_tiny(float);*/
int ifsign(float);
void place_in_bounds(float *, float *, float *);
void round_to_bignum(float *, float *, float *);
float find_voxel_edge(int, float);
unsigned char geom_array_lookup(geom_info_t *, int, int, int);
unsigned char lookup_nochecking(int, int, int);
void ndcf_to_wcf(geom_info_t *, float, float, float,
		 float *, float *, float *);
void wcf_to_ndcf(geom_info_t *,
		 float, float, float,
		 float *, float *, float *);
unsigned char get_mapping(geom_info_t *, char);
unsigned char add_line(geom_info_t *, unsigned char, float, float, float, float, int);
void fill_region_driver(geom_info_t *, int, int, int, unsigned char, unsigned char);
void fill_region(geom_info_t *, int, int, int, unsigned char, unsigned char, unsigned char *);
void fill_region_wvect(geom_info_t *, float, float, float, float, int, unsigned char, unsigned char);
void float_sort(float *, int);
void float_sort_3(float *);
void float_sort_4(float *);
void enlarge_bounding_box_simple(geom_info_t *);
void enlarge_bounding_box(geom_info_t *, int, int, int);
void print_tracking_info(geom_info_t *,
			 int, int, int,
			 double,
			 float, float, float,
			 float, float, float);
...
geom_info_t ggg;  /* GLOBAL geometry info */
...
int global_accuracy = 1;     /* 1 means accurate, 0 means less accurate */
int total_intersections = 0; /* global to keep track of number of
 ...
 */
int bitregions[8] = {1, 2, 4, 8, 16, 32, 64, 128};
...
int process_region_property(int uvvalue, geom_info_t * geom_ptr,
  ...
  if ((caseval==-1)||(uvvalue<0)||(uvvalue>255)) return(0);
  
  /* mwf032498  printf("  Inside process_region_property...\n"); */
  /* mwf032498  printf("  --> Processing:  %s:%s\n", key, value); */

  e_keyval = caseval;

  /* printf("for %d:  ", uvvalue); */
  ...
  switch(e_keyval)
	...
	geom_ptr->bodyname[uvvalue][255]='\0';
	strncpy(geom_ptr->bodyname[uvvalue], value, 255);
      }
    ...
    case e_matname: /* matname */
      if (strlen(value)<255) {
	strcpy(geom_ptr->matname[uvvalue], value);
      } else {
	geom_ptr->matname[uvvalue][255]='\0';
	strncpy(geom_ptr->matname[uvvalue], value, 255);
      }
      geom_ptr->valid.matname[uvvalue]=1;
      break;
	...
	geom_ptr->parent_body[uvvalue][255]='\0';
	strncpy(geom_ptr->parent_body[uvvalue], value, 255);
      }
 ...
 * reported by STRLEN.
 ******************************************************************/
int UV_readln(FILE *fptr, char *s, int maxsize) {
  int i=0, ch;
...
/******************************************************************
 * UV_readln_skip_comments_and_trim
 ******************************************************************
 ...
 *   * in file
 ******************************************************************/
int UV_readln_skip_comments_and_trim(FILE *fptr, char *s, int maxsize) {
  int length;
  char * loc;
...
/******************************************************************
 * UV_break_into_lower_key_and_value
 ******************************************************************
 ...
 ******************************************************************/
void UV_break_into_lower_key_and_value(char * br, char * s,
				       char ** key, char ** value) {
  char * wherebreak;
 ...
 ******************************************************************/
int UV_read_next_key_and_value(FILE * fptr, char * str, int maxl, char ** key, char ** value) {
  int len;
...
/******************************************************************
 * UV_read_string_value_for_key
 ******************************************************************
 * rewinds the file, searches for first occurance of key,
 * returns value of the key
 * Returns 1 if it's found, 0 if it's not
 ******************************************************************/
int UV_read_string_value_for_key(FILE * fptr, char * key, char * value, int maxl) {
 ...
 * for the body.
 ******************************************************************/
int read_uvval_for_body(FILE * fptr, char * bodyname) {
  int uvvalue;
 ...
 ******************************************************************/
void read_body_information(FILE * fptr, geom_info_t * geom_ptr,
			   char * bodyname) {
  int uvval;
  char str[MAX_UVH_LINE_SIZE], * key, * value;
   ...
   * return at this time
   */
  if ((uvval<0)||(uvval>255)) return;

  process_region_property(uvval, geom_ptr, UVH_BEGIN, bodyname);

  while (UV_read_next_key_and_value(fptr, str, MAX_UVH_LINE_SIZE,
				 &key, &value)) {
 /* mwf032498 printf("Looping in read_body_information...\n"); */
 /* mwf032498 printf("  and got this key and value:  %s:%s\n", key, value);*/
    if (!strcmp(key, UVH_END)) break;
    /* mwf032498  printf("  Will try to process the pair...\n"); */
 ...
 * the marker.
 ******************************************************************/
void read_constraint_information(FILE * fptr, geom_info_t * geom_ptr,
			   char * markername) {
  char str[MAX_UVH_LINE_SIZE], * key, * value;

  printf("read_constraint_information is stubbed.\n");
  while (UV_read_next_key_and_value(fptr, str, MAX_UVH_LINE_SIZE,
				 &key, &value)) {
    if (!strcmp(key, UVH_END)) break;
  }
  printf("End found for constraint %s\n", markername);
}


/******************************************************************
 * read_fiducial_information
 ******************************************************************
 * Just read a line from the uvh file indicating to start a new
 ...
 * the marker.
 ******************************************************************/
void read_fiducial_information(FILE * fptr, geom_info_t * geom_ptr,
			   char * markername) {
  char str[MAX_UVH_LINE_SIZE], * key, * value;

  printf("read_fiducial_information is stubbed.\n");
  while (UV_read_next_key_and_value(fptr, str, MAX_UVH_LINE_SIZE,
				 &key, &value)) {
    if (!strcmp(key, UVH_END)) break;
  }
  printf("End found for fiducial %s\n", markername);
}


/******************************************************************
 * UV_make_lower_string
 ******************************************************************
 * Given a string, converts all uppercase characters found to
 * corresponding lowercase characters
 ******************************************************************/
void UV_make_lower_string(char *s) {
  int i;
  char ch;
 ...
 ******************************************************************/
void place_in_bounds(float *x, float *y, float *z) {
 ...
 ******************************************************************/
void round_to_bignum(float *x, float *y, float *z) {
  return;
 ...
 ******************************************************************/
float find_voxel_edge(int x, float v) {
  if (v<0.0) {
    return((float)(x+1));
  } else {
    return((float)x);
 ...
 ******************************************************************/
unsigned char geom_array_lookup(geom_info_t *geom, int x, int y, int z) {
 ...
 * *** CAN ONLY BE USED AFTER CALLING INIT RAY ENVIRONMENT ***
 ******************************************************************/
unsigned char lookup_nochecking(int x, int y, int z) {
 ...
 ******************************************************************/
void ndcf_to_wcf(geom_info_t * geom, float xf, float yf, float zf,
 ...
 ******************************************************************/
void wcf_to_ndcf(geom_info_t * geom,
 ...
 * in the output file
 ******************************************************************/
unsigned char get_mapping(geom_info_t * geom_ptr, char ch) {
  char low_bit;
  char high_bit;
  int i;
 ...
 ******************************************************************/
void fill_region_driver(geom_info_t *geom_ptr, int x, int y, int z, unsigned char fillbit, unsigned char color_to_replace) {
 ...
 ******************************************************************/
void fill_region(geom_info_t *geom_ptr, int x, int y, int z, unsigned char fillbit, unsigned char color_to_replace, unsigned char *buffer) {
 ...
 ******************************************************************/
void fill_region_wvect(geom_info_t *geom, float xstart, float ystart, float xvect, float yvect, int planeno, unsigned char regbit, unsigned char color_to_replace) {
  int xpt, ypt;
 ...
 ******************************************************************/
void float_sort(float * arr, int n) {
  static int i, j;
  static float tmp;
  static int not_done;
...
void initialize_geom_info(geom_info_t * geom_ptr) {
  ...
  geom_ptr->fiducial_markers_list = NULL;
  geom_ptr->constraint_markers_list = NULL;

  v->num_fiducial_markers = 1;
  v->num_constraint_markers = 1;
  v->fiducial_markers_list = 1;
  v->constraint_markers_list = 1;
 ...
 ******************************************************************/
FILE * read_uvh_minimal_fptr(geom_info_t * geom_ptr, char *headerfile, int leave_open) {
  int i;
  FILE *fptr;
  ...
  if (!UV_read_string_value_for_key(fptr, UVH_SLICEORIENTATION, value, MAX_UVH_LINE_SIZE)) {
    ...
    if (!strcmp(value, "transverse")) {
      ...
      fprintf(stderr, "Unacceptable slice orienatation in .uvh file.\n");
...
void write_uvh(geom_info_t * geom_ptr, char *name) {
  char outname[256];
  int i, total_pixels;
  char uvinfo[8192]; /* DON'T EXCEED THIS FOR ANY UVVAL */
...
void enlarge_bounding_box_simple(geom_info_t * geom_ptr) {
    /* This will add 20 to width, 20 to height, and 4 to imageslices */
    enlarge_bounding_box(geom_ptr, 10, 10, 2);
}

void enlarge_bounding_box(geom_info_t * geom_ptr, int add_half_x, int add_half_y, int add_half_z) {
...
void process_resource_file(geom_info_t * geom_ptr, int * accuracy, float * enlarge) {
  ...
  *enlarge = 0.0;
  ...
  ResourcePath = (char *)getenv("BNCT_RESOURCES");
  if ((ResourcePath)&&(strlen(ResourcePath)>0)) {
      strcpy(name, ResourcePath);
  } else {
      fprintf(stderr, "\nBNCT_RESOURCES environment variable not set.\n\
 ...
 */
void print_tracking_info(geom_info_t * geom_ptr,
    ...
    printf("Start %0.2f %0.2f %0.2f:  %s\n", wcf_x0, wcf_y0, wcf_z0,
	   geom_ptr->bodyname[initial]);
    if (!miss) {
	printf("End   %0.2f %0.2f %0.2f:  %s\n", wcf_x, wcf_y, wcf_z,
	       geom_ptr->bodyname[final]);
	       ...
	       int *this, int *next, int *miss )
{
    ...
    region_t last_material=0, material;
    ...
    *miss = 1;
    ...
    material = geom_array_lookup(geom_ptr, int_rnd_down(ndcf_x0),
    ...
    /* If the material is 0, we start _outside_ the box
     * Options are to:
	...
	if ((!material)&&(!last_material)) {	    
	     ...
	     * a miss -- say ray started in buffer.
	     */
	    *this = geom_ptr->regionnum
		[geom_ptr->uvval[1]];
#ifdef PRINTS
	...
	/* If initial material was 0, we started in 'buffer' outside
	 * of the bounding box.  Need to make a few modifications
	 * to convert '0' into 'buffer'.
	 */
	if (material==0) {
		...
		*next = geom_ptr->regionnum[last_material];
		*miss = 0;
    ...
    *this = geom_ptr->regionnum[material];
	...
	switch(varies_most)
	    ...
	    case 1:
	    case 2:
		material = geom_array_lookup(geom_ptr, xx, yy, zz);
		break;
	    case 3:
	    case 4:
		material = geom_array_lookup(geom_ptr, yy, xx, zz);
		break;
	    case 5:
	    case 6:
		material = geom_array_lookup(geom_ptr, zz, yy, xx);
		break;
	    default:
		fprintf(stderr, "Error!  Error!\n");
	    }
...
#endif
    /* If material is 0, we've exited the box.  'buffer' to box exit
     * is simply reported as a miss.  If we're in any other material
     * when we exit the box, we must report at which point we exit
     * the box and say the next material is 'buffer'.
     */
    if ((material==0)&&(last_material==1)) {
#ifdef PRINTS
	print_tracking_info(geom_ptr, *this, *next, *miss, *dist,
			    wcf_x0, wcf_y0, wcf_z0,
			    wcf_x,  wcf_y,  wcf_z);
	...
	return;
	      ...
	      * is 0 and last_material wasn't buffer
	      */
	...
	for (i=si; i<4; i++) {
		...
		if (last_material!=
		    (material=geom_array_lookup(geom_ptr,
						int_rnd_down(ndcf_x),
						int_rnd_down(ndcf_y),
						int_rnd_down(ndcf_z)))) {
		    success=1;
...
occur) by indicating a miss.\n");
    ...
    /* Never report material 0.  0 only means we're outside of the
     * bounding box.  By definition, everything outside of the
     * bounding box is 'buffer' or material 1.
     */
    if (material==0) {
	material=1;
	if (last_material==1) {
	    /* In this case, went from 'buffer' to outside of bounding
	     * box and was going to return the intersection point.
	     * Instead, we need to indicate a miss.
	     */
#ifdef PRINTS
	    print_tracking_info(geom_ptr, *this, *next, *miss, *dist,
				wcf_x0, wcf_y0, wcf_z0,
