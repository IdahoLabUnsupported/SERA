...
int	coords;		/* Number of coordsinates for mat_2 and solution */
{
	...
	y = (fastf_t *) malloc(sizeof (fastf_t) * dim);/* Create temp array */

	b = (fastf_t *) malloc(sizeof (fastf_t) * dim);/* Create temp array */

	s = (fastf_t *) malloc(sizeof (fastf_t) * dim);/* Create temp array */
	...
	free (y);			/* Free up storage */
	free (b);			/* Free up storage */
	free (s);			/* Free up storage */
}
...
int coords;
{
	...
	d = (fastf_t * ) malloc( sizeof (fastf_t) * row);	/* scale factor */
	s = (fastf_t * ) malloc( sizeof (fastf_t) * row * row);	/* vector to check */
	ds = (fastf_t *) malloc( sizeof (fastf_t) * row);	/* if rows need to be swaped */
