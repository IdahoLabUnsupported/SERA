...
void
rt_nurb_kvknot( register struct knot_vector *new_knots, 
		int order, 
		fastf_t lower, 
		fastf_t upper, 
		int num)
...
*/
{
	...
	new_knots->knots = (fastf_t * ) malloc ( sizeof( fastf_t) * total);
 ...
 */
void
rt_nurb_kvmult( struct knot_vector *new_kv, 
...
/*
	...
	check.knots = (fastf_t * ) malloc( sizeof(fastf_t) * check.k_size);
...
void
rt_nurb_kvgen( register struct knot_vector *kv, 
	       fastf_t lower, 
	       fastf_t upper, 
	       int num)
/*
...
*/
{
	...
	kv->k_size = num;
	kv->knots = (fastf_t * ) malloc ( sizeof( fastf_t) * num);
...
CONST struct knot_vector *kv2;
{
	...
	new_knots->knots = (fastf_t * ) malloc( 
...
register CONST struct knot_vector *old_kv;
{
	...
	new_kv->knots = (fastf_t * ) malloc( sizeof( fastf_t) * 
	    new_kv->k_size);
...
void
rt_nurb_gen_knot_vector( register struct knot_vector *new_knots, 
			 int order, 
			 fastf_t lower, 
			 fastf_t upper)
/*
