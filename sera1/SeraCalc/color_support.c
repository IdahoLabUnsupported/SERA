...
{
    int i, j, src_width, src_height, dst_pos, src_pos, eoln_incr, bytes;
...
{
    viewGui_t *gui = (viewGui_t *)clientdata;
...
void register_colormap_ehs_for_widget( viewGui_t * gui, Widget w, Colormap cmap )
{
    ...
    if (XtIsRealized(w)) {
        if (gui->color_info.maxHWcolormaps == 1){
            ...
            XSetWMColormapWindows(gui->display, XtWindow( rtt_top ), &wi, 1);
...
void init_color(viewGui_t *gui, int r, int g, int b, XColor *color)
{
    ...
    color->blue = b;
    color->flags = DoRed | DoGreen | DoBlue;
    XAllocColor(gui->display,cmap, color);
